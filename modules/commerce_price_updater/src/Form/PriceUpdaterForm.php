...
class PriceUpdaterForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['csv_file'] = [
      ...
      '#required' => TRUE,
      '#description' => $this->t('CSV format: SKU, PRICE. Replace comma with your separator. <a href="/multty/sites/default/files/product_price_update.csv">Click here</a> to download format'),
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    while ($line = fgetcsv($handle, 4096, $separator)) {
      ...
      if ($sku && $price && is_numeric($price)) {
        ...
        $counter++;
      } elseif (strcmp($price, 'Price') != 0) {
        $error++;
        drupal_set_message($this->t('This price format "'. $price .'" is invalid'), 'error');
      }

      $storage = $this->entityTypeManager->getStorage('commerce_product_variation');
      $product_variations = $storage->loadByProperties(['sku' => $sku]);
      if (!$product_variations && (strcmp($sku, 'SKU') != 0)) {
        $error++;
        drupal_set_message($this->t('SKU "'. $sku .'" is not found'), 'error');
      }
    ...
    if ($counter > 0) {
      batch_set($batch);
      if ($errorCounter > 0) {
        drupal_set_message($this->t('There were '. $errorCounter .' error(s). Please check your CSV file'), 'error');
      }
      drupal_set_message($this->t($updated.' product prices were successfully updated.'));
    }
    else {
