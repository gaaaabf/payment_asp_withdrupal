...
class CustomerProfile extends EntityInlineFormBase {
  ...
  protected function buildOptions(array $address_book_profiles) {
    ...
    if (isset($profile_options)) {
      foreach($profile_options as $key => $value){
        $storage = \Drupal::entityTypeManager()->getStorage('profile');
        $profile = $storage->load($key);
        $address = $profile->get('address')->getValue();
        $profile_options[$key] = $address[0]['address_line1'].' '.$address[0]['address_line2'].' '.$address[0]['administrative_area'].' '.$address[0]['postal_code'].' '.$address[0]['organization'].' '.$address[0]['country_code'];
      }
    }
