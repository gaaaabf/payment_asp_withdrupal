...
class CleantalkFuncs {
	...
	*/
	...
	static public function _cleantalk_get_submit_time()	{
		...
		return self::_cleantalk_apbct_cookies_test() == 1 ? time() - (int)\Drupal::request()->cookies->get('apbct_ps_timestamp') : null;
	...
	*/
	...
	static public function _cleantalk_check_spam($spam_check, $form_errors = null) {
		...
		$sender_info = \Drupal\Component\Serialization\Json::encode(
            ...
            'form_validation' => ($form_errors && is_array($form_errors)) ? json_encode(array('validation_notice' => strip_tags(json_encode($form_errors)), 'page_url' => $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'])) : null,
			)
		);
