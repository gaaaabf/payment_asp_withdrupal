...
class AddressCheckoutHelper {
  ...
  public function ajaxGetAddress($form_id, &$form, &$form_state) {
    ...
    foreach ($profiles as $key => $value) {
      $profilesData[$key] = $value->get('address')->getString();
    }
    ...
    $defaultprofileid = reset($profile)->id();
    if($defaultprofileid) {
      $defaultprofile = Profile::load($defaultprofileid);
    }
    $defaultValue = $this->request->getParentRequest()->get('id', NULL);
    // Check default value.
    ...
    elseif($defaultValue == NULL) {
      if($defaultprofile) {
        $form['payment_information']['billing_information']['#default_value'] = $defaultprofile;
